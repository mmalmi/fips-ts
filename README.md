...
Browser TypeScript implementation of [FIPS](https://github.com/jmcorgan/fips) — node identity, FMP / FSP, WebRTC datachannel transport, Nostr signaling (NIP-44 v2 + NIP-59 gift wrap), service-port datagrams, and a hashtree adapter.
...
packages/hashtree-adapter  Service-port-7001 adapter speaking @hashtree/mesh
                           MessagePack DataRequest / DataResponse frames
...
pnpm test:unit         # 39 vitest tests
pnpm test:e2e          # 9 playwright tests (chromium)
...
hashtree → FipsHashtreeStore → FIPS service port 7001
              ...
              → WebRTC datachannel (or MemoryTransport in tests)
