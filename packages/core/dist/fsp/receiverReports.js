...
export class FspReceiverReports {
    ...
    record(received, nowMs, currentEpoch = true) {
        this.packets++;
        ...
        if (received.counter < this.highest)
            this.reordered = (this.reordered + 1) >>> 0;
        else
            this.highest = received.counter;
        ...
        if (this.transitSamples.length > 32)
            this.transitSamples.shift();
        this.timestamp = received.timestamp;
        this.receivedAt = nowMs;
    }
    build(nowMs) {
        if (this.intervalPackets === 0 || this.receivedAt === undefined)
            return undefined;
        this.finishBurst();
