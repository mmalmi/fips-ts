...
export class FmpTransportPacketProcessor {
    ...
    stageUnconfirmedCarrier(peer, handshakeLink) {
        if (peer.link !== handshakeLink) {
            return peer.link.state !== "established" && peer.pendingResponderLink === handshakeLink;
        }
        if (!this.establishedRemoteEpoch(peer.pubkeyHex, handshakeLink))
            return false;
    ...
    handleMsg2(transport, remoteAddr, addressedPeer, packet) {
        ...
        if (!wasEstablished) {
            ...
            void peer.transport.send(peer.remoteAddr, peer.link.encryptOutgoing(new Uint8Array(0), FMP_INNER_KEEPALIVE)).catch((error) => this.cfg.emitError(error, "confirm FMP handshake"));
            this.cfg.emitPeer({
                remotePubkey: peer.pubkeyHex,
                remoteAddr: peer.remoteAddr,
                state: "connected",
