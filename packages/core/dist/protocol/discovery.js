...
import { nodeAddrFromSlice } from "../nodeaddr/index.js";
export function decodeLookupRequest(payload) {
    ...
    if (reader.remaining !== 0)
        throw new Error("trailing LookupRequest bytes");
    return request;
}
export function encodeLookupRequestPayload(request) {
...
export function decodeLookupResponse(payload) {
    const reader = new BinaryReader(payload);
    const response = {
        requestId: reader.u64le(),
        target: nodeAddrFromSlice(reader.bytes(16)),
    ...
    if (reader.remaining !== 0)
        throw new Error("trailing LookupResponse bytes");
    return response;
}
export function encodeLookupResponsePayload(response) {
    if (response.proof.length !== 64)
        throw new Error("LookupResponse proof must be 64 bytes");
    const writer = new BinaryWriter();
...
export function lookupResponseProofBytes(requestId, target, targetCoords) {
    const writer = new BinaryWriter();
...
function decodeCoords(reader) {
    const count = reader.u16le();
    if (count === 0)
        throw new Error("lookup coordinates must not be empty");
    const coords = [];
...
function encodeCoords(writer, coords) {
    ...
    for (const coord of coords) {
        if (coord.length !== 16)
            throw new Error("lookup coordinate must be 16 bytes");
        writer.bytes(coord);
    }
}
