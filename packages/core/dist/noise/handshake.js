...
import { ecdh } from "../identity/index.js";
...
function deriveKeyPair(priv) {
    return { priv, pub: secp256k1.getPublicKey(priv, true) };
}
function genKeyPair(override) {
    if (override) {
        if (override.length !== 32)
            throw new Error("ephemeral override must be 32 bytes");
...
export class NoiseHandshake {
    ...
    constructor(init) {
        ...
        const responderStatic = this.role === "responder" ? this.s.pub : this.rs;
        if (!responderStatic) {
    ...
    readMessage(message) {
        ...
        const payload = remaining.length > 0 ? this.ss.decryptAndHash(remaining) : new Uint8Array(0);
        this.step += 1;
        return payload;
    }
