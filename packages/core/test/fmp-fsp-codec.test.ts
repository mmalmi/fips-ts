...
describe("FMP wire codec", () => {
  it("Msg1 round-trip is 114 bytes with phase=1 and payload_len=106", () => {
    ...
    // payload_len is u16 LE at offset 2-3 == 106 (0x6a 0x00)
    expect(msg1[2]).toBe(106);
  ...
  it("Msg2 round-trip is 69 bytes with phase=2 and payload_len=57", () => {
...
describe("FSP wire codec", () => {
  ...
  it("FSP Established has 12-byte header (no receiver_idx)", () => {
    ...
    const p = encodeFspEstablished({ flags: 0, counter: 7n, ciphertext: ct });
