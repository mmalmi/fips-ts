...
export const RECENT_PEERS_VERSION = 1 as const;
...
interface ParsedUdpSocketAddr {
  addr: string;
  canonical: string;
  key: string;
}

export function createRecentPeers(localNpub: string, scope: string): RecentPeers {
...
export function parseRecentPeers(
  value: unknown,
  expectedLocalNpub: string,
  ...
  const peers: Record<string, RecentPeer> = {};
...
export function observeAuthenticatedPeer(
  recentPeers: RecentPeers,
  remoteNpub: string,
  authenticatedAtMs: number,
  udpAddr?: string,
): RecentPeers {
  const parsed = parseRecentPeers(
    recentPeers,
  ...
  const lastAuthenticatedAtMs = Math.max(
    existing?.last_authenticated_at_ms ?? 0,
    authenticatedAtMs,
  );
  let endpoints = existing?.endpoints ?? [];
  if (parsedUdpAddr !== undefined) {
    const previous = endpoints.find((endpoint) => (
      requireReusableUdpSocketAddr(endpoint.addr, "cached UDP addr").key === parsedUdpAddr.key
    ));
    ...
    endpoints.push({
      ...
      last_authenticated_at_ms: Math.max(
        previous?.last_authenticated_at_ms ?? 0,
        authenticatedAtMs,
      ),
    });
  }
...
export function pruneRecentPeers(
  recentPeers: RecentPeers,
  nowMs: number,
  ttlMs: number,
): RecentPeers {
  ...
  for (const [remoteNpub, peer] of Object.entries(parsed.peers)) {
    if (peer.last_authenticated_at_ms < cutoff) continue;
    peers[remoteNpub] = {
...
function parseRecentPeer(value: unknown): RecentPeer {
  const peer = requireRecord(value, "recent peer");
  requireExactKeys(peer, ["last_authenticated_at_ms", "endpoints"], "recent peer");
  const lastAuthenticatedAtMs = requireTimestamp(
    peer.last_authenticated_at_ms,
    "recent peer last_authenticated_at_ms",
  );
  if (!Array.isArray(peer.endpoints)) throw new Error("recent peer endpoints must be an array");
  ...
  const endpoints = peer.endpoints.map((endpoint) => parseRecentPeerEndpoint(
    endpoint,
    lastAuthenticatedAtMs,
  ));
  const addresses = new Set(endpoints.map(({ addr }) => (
    requireReusableUdpSocketAddr(addr, "recent peer endpoint addr").key
  )));
...
function parseRecentPeerEndpoint(value: unknown, peerAuthenticatedAtMs: number): RecentPeerEndpoint {
  const endpoint = requireRecord(value, "recent peer endpoint");
  requireExactKeys(
    endpoint,
    ["transport", "addr", "last_authenticated_at_ms"],
    "recent peer endpoint",
  );
  if (endpoint.transport !== "udp") {
    throw new Error("recent peer endpoint transport must be udp");
  }
  const addr = requireReusableUdpSocketAddr(
    endpoint.addr,
    "recent peer endpoint addr",
  ).addr;
  const lastAuthenticatedAtMs = requireTimestamp(
    endpoint.last_authenticated_at_ms,
    "recent peer endpoint last_authenticated_at_ms",
  );
...
function capPeers(peers: Record<string, RecentPeer>): Record<string, RecentPeer> {
  return Object.fromEntries(
    Object.entries(peers)
      .sort(([leftNpub, left], [rightNpub, right]) => (
        right.last_authenticated_at_ms - left.last_authenticated_at_ms
...
function clonePeers(peers: Record<string, RecentPeer>): Record<string, RecentPeer> {
...
function compareEndpoints(left: RecentPeerEndpoint, right: RecentPeerEndpoint): number {
...
function requireReusableUdpSocketAddr(value: unknown, label: string): ParsedUdpSocketAddr {
...
function parseIpv4SocketAddr(addr: string): Omit<ParsedUdpSocketAddr, "addr"> | undefined {
  ...
  if (octetStrings.length !== 4) return undefined;
  const octets = octetStrings.map((octet) => Number(octet));
  if (octetStrings.some((octet, index) => (
    !/^(0|[1-9][0-9]{0,2})$/.test(octet)
    || !Number.isInteger(octets[index])
    || octets[index]! > 255
  ))) return undefined;
  if (octets.every((octet) => octet === 0)) return undefined;
  if (octets[0]! >= 224 && octets[0]! <= 239) return undefined;

  const canonical = `${octets.join(".")}:${port}`;
  return { canonical, key: canonical };
}

function parseIpv6SocketAddr(addr: string): Omit<ParsedUdpSocketAddr, "addr"> | undefined {
  const closingBracket = addr.indexOf("]");
  if (closingBracket <= 1 || addr[closingBracket + 1] !== ":") return undefined;
  ...
  if (scopeSeparator !== hostAndScope.lastIndexOf("%")) return undefined;
  ...
  try {
    ...
    if (!urlHost.startsWith("[") || !urlHost.endsWith("]")) return undefined;
    normalizedHost = urlHost.slice(1, -1).toLowerCase();
  } catch {
    return undefined;
  }
  if (normalizedHost === "::" || normalizedHost.startsWith("ff")) return undefined;
...
function parseReusablePort(value: string): number | undefined {
...
function parseIpv6Scope(value: string): number | undefined {
  if (!/^[0-9]+$/.test(value)) return undefined;
...
function requireCanonicalNpub(value: unknown, label: string): string {
  ...
  try {
    if (encodeNpub(decodeNpub(npub)) !== npub) throw new Error("non-canonical npub");
  } catch {
...
function requireNonEmptyString(value: unknown, label: string): string {
  ...
  if (stringValue.length === 0) throw new Error(`${label} must not be empty`);
  return stringValue;
}
