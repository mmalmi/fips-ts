...
export function encodeForwardEnvelope(e: ForwardEnvelope): Uint8Array {
  if (e.srcPubkey.length !== 33) throw new Error("src pubkey must be 33");
  if (e.dstPubkey.length !== 33) throw new Error("dst pubkey must be 33");
  if (e.version !== FORWARD_VERSION) throw new Error("bad forward version");
...
export function decodeForwardEnvelope(buf: Uint8Array): ForwardEnvelope {
  if (buf.length < 2 + 33 + 33) throw new Error("forward envelope too short");
  const r = new BinaryReader(buf);
  const version = r.u8();
  if (version !== FORWARD_VERSION) throw new Error("not a FORWARD envelope");
  const ttl = r.u8();
