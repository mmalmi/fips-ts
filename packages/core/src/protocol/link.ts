...
export function handshakeMessageTypeFromByte(b: number): number | undefined {
  if (b === 0x01 || b === 0x02) return b;
  return undefined;
}

export function isHandshakeMessageType(b: number): boolean {
...
const LINK_MESSAGE_TYPE_BYTES = new Set<number>([
...
export function linkMessageTypeFromByte(b: number): number | undefined {
...
const DISCONNECT_REASON_BYTES = new Set<number>([
...
export function disconnectReasonFromByte(b: number): number {
...
export function encodeDisconnect(d: Disconnect): Uint8Array {
 ...
 * `Disconnect::decode`).
 */
export function decodeDisconnect(payload: Uint8Array): Disconnect {
  if (payload.length < 1) {
...
export interface SessionDatagram {
  ttl: number;
  pathMtu: number;
  srcAddr: Uint8Array;   // 16-byte NodeAddr
  destAddr: Uint8Array;  // 16-byte NodeAddr
  payload: Uint8Array;
}

export const SESSION_DATAGRAM_HEADER_SIZE = 36;

export function encodeSessionDatagram(d: SessionDatagram): Uint8Array {
  if (d.srcAddr.length !== 16) throw new Error("src_addr must be 16 bytes");
  if (d.destAddr.length !== 16) throw new Error("dest_addr must be 16 bytes");
...
export function decodeSessionDatagram(buf: Uint8Array): SessionDatagram {
  if (buf.length < SESSION_DATAGRAM_HEADER_SIZE) {
    throw new Error(
      `SessionDatagram too short: ${buf.length} < ${SESSION_DATAGRAM_HEADER_SIZE}`,
    );
  }
...
export function decodeSessionDatagramPayload(buf: Uint8Array): SessionDatagram {
  if (buf.length < SESSION_DATAGRAM_HEADER_SIZE - 1) {
    throw new Error(
      `SessionDatagram payload too short: ${buf.length} < ${SESSION_DATAGRAM_HEADER_SIZE - 1}`,
    );
  }
  const r = new BinaryReader(buf);
  const ttl = r.u8();
...
export function decrementTtl(d: SessionDatagram): boolean {
  if (d.ttl <= 1) {
    d.ttl = 0;
    return false;
  }
