...
function expectedFilterBytes(sizeClass: number): number {
 ...
 */
export function buildFilterAnnounce(
  filter: BloomFilter,
  sequence: bigint,
): FilterAnnounce {
  if (filter.numBytes !== expectedFilterBytes(V1_SIZE_CLASS)) {
    throw new Error(
      `v1 FilterAnnounce requires ${expectedFilterBytes(V1_SIZE_CLASS)}-byte filter (got ${filter.numBytes})`,
    );
  }
  return {
    filter,
...
export function encodeFilterAnnounce(fa: FilterAnnounce): Uint8Array {
  if (fa.filter.numBytes !== expectedFilterBytes(fa.sizeClass)) {
    throw new Error(
      `filter size ${fa.filter.numBytes} doesn't match size_class ${fa.sizeClass}`,
    );
  }
  if (fa.hashCount !== fa.filter.hashCount) {
    throw new Error(
      `hash_count ${fa.hashCount} doesn't match filter.hashCount ${fa.filter.hashCount}`,
    );
  }
 ...
 */
export function decodeFilterAnnounce(buf: Uint8Array): FilterAnnounce {
  if (buf.length < 1 + FILTER_ANNOUNCE_MIN_PAYLOAD_SIZE) {
    throw new Error(
      `FilterAnnounce too short: ${buf.length} < ${1 + FILTER_ANNOUNCE_MIN_PAYLOAD_SIZE}`,
    );
  }
  ...
  if (r.remaining < filterLen) {
    throw new Error(
      `FilterAnnounce truncated: need ${filterLen} filter bytes, have ${r.remaining}`,
    );
  }
