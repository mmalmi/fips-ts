...
const conversationKeyCache = new WeakMap();
function getConversationKey(identity, peerXOnlyHex) {
...
export function encryptSignalContent(identity, recipientXOnlyHex, plaintext) {
    const ck = getConversationKey(identity, recipientXOnlyHex);
    return nip44v2.encrypt(plaintext, ck);
}
export function decryptSignalContent(identity, senderXOnlyHex, content) {
    const ck = getConversationKey(identity, senderXOnlyHex);
    return nip44v2.decrypt(content, ck);
}
//# sourceMappingURL=signalEncryption.js.map